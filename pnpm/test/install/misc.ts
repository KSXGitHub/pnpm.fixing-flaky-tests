...
skipOnWindows('install --lockfile-only', async () => {
  ...
  // is-positive@1.0.0 is pre-seeded in the Verdaccio registry mock and has no
  // transitive npm dependencies, making the test resilient to proxy failures.
  // rimraf@2.5.1 was previously used but its dependency glob@6.0.4 requires
  // minimatch@2.x which is not available in the registry mock without proxying
  // to npm, causing intermittent ERR_PNPM_FETCH_404 failures on CI.
