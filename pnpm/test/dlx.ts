...
test('dlx creates cache and store prune cleans cache', async () => {
  ...
  await Promise.all(Object.entries(commands).map(([cmd, args]) =>
    execPnpm([...settings, '--allow-build=shx', 'dlx', cmd, ...args],
      // Git-hosted packages require cloning the repository, which can take longer than
      // the default 3-minute timeout on a slow CI runner. Use 5 minutes instead.
      { timeout: 5 * 60 * 1000 })))
